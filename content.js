...
function createSuggestionElements() {
  ...
  suggestionPopup.querySelector('.twitter-improver-popup-close').addEventListener('click', () => {
    suggestionPopup.style.display = 'none';
  });
  
  // Fixed apply suggestion functionality
  suggestionPopup.querySelector('.twitter-improver-apply').addEventListener('click', () => {
    const suggestedText = suggestionPopup.querySelector('.twitter-improver-suggested-text')?.textContent?.trim();
    if (suggestedText) {
...
function applyTextSafely(text) {
  ...
  try {
    ...
    if (currentEditingElement.isContentEditable) {
      ...
      const existingSpan = currentEditingElement.querySelector('span[data-offset-key]');
      const offsetKeyBase = existingSpan ? existingSpan.getAttribute('data-offset-key').split('-')[0] : generateOffsetKey();
      
      // Clear existing content first while preserving the root structure
      const rootDiv = currentEditingElement.querySelector('[data-contents="true"]');
      if (rootDiv) {
        rootDiv.innerHTML = '';
        ...
        blockDiv.setAttribute('data-editor', currentEditingElement.getAttribute('data-editor') || '');
    ...
    else if (currentEditingElement.tagName === 'INPUT' || currentEditingElement.tagName === 'TEXTAREA') {
      currentEditingElement.value = text;
      currentEditingElement.dispatchEvent(new Event('input', { bubbles: true }));
      currentEditingElement.dispatchEvent(new Event('change', { bubbles: true }));
    }
    ...
    navigator.clipboard.writeText(text).then(() => {
      alert('Cannot apply directly. Text copied to clipboard - please paste manually.');
      suggestionPopup.style.display = 'none';
    }).catch(err => {
...
function toggleSuggestionPopup() {
  if (suggestionPopup.style.display === 'none') {
    ...
    suggestionPopup.style.left = `${buttonRect.left + window.scrollX - 150 + buttonRect.width/2}px`;
...
function showSuggestionButton() {
  ...
  suggestionButton.style.top = `${rect.top + window.scrollY + rect.height/2 - 12}px`;
...
function analyzeCurrentText() {
  ...
  // Show loading state
  suggestionPopup.querySelector('.twitter-improver-loading').style.display = 'flex';
  suggestionPopup.querySelector('.twitter-improver-suggestion-content').style.display = 'none';
  suggestionButton.classList.add('twitter-improver-analyzing');
...
async function improveText(text) {
  try {
    const prompt = `Generate a Twitter post that sounds like a casual conversation, with human-like imperfections. Use short sentences, include some grammatical mistakes, and avoid over-polishing the text. Keep it punchy, natural, and easy to read. The tone should feel conversational, not overly formal. Don't add too much grammar structure, just make it feel like a regular tweet from a person. Keep it short like 8-11 words per line. Original post: "${text}"`;
    ...
    const suggestion = data?.candidates?.[0]?.content?.parts?.[0]?.text || "No improvement available.";
    
    // Update popup with suggestion
    suggestionPopup.querySelector('.twitter-improver-suggested-text').textContent = suggestion;
    
    showSuggestionContent();
  } catch (error) {
    console.error("Error calling Gemini API:", error);
    suggestionPopup.querySelector('.twitter-improver-suggested-text').textContent = 
...
function showSuggestionContent() {
  suggestionPopup.querySelector('.twitter-improver-loading').style.display = 'none';
  suggestionPopup.querySelector('.twitter-improver-suggestion-content').style.display = 'block';
}
...
function isTweetInputField(element) {
  ...
  if (element.getAttribute('aria-label') === 'Tweet text' || 
      element.getAttribute('aria-label') === 'Post text') return true;
  if (element.getAttribute('role') === 'textbox' && 
      (element.closest('[data-testid="tweetTextarea_0"]') || 
       element.closest('[data-testid="tweetTextarea"]'))) return true;
  ...
  if (element.isContentEditable && 
      (element.closest('[data-testid="tweetTextarea_0"]') || 
       ...
       element.closest('[aria-label="Post text"]'))) return true;
  
  // More specific Twitter selectors
  if (element.matches('[data-testid="tweetTextarea_0"] [contenteditable="true"]') ||
      element.matches('[data-testid="tweetTextarea"] [contenteditable="true"]')) return true;
...
function setupTweetObserver() {
  ...
  document.addEventListener('click', (e) => {
    ...
    const tweetInputs = document.querySelectorAll('[data-testid="tweetTextarea_0"] [contenteditable="true"], [data-testid="tweetTextarea"] [contenteditable="true"], [aria-label="Tweet text"], [aria-label="Post text"], [role="textbox"]');
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter' && currentEditingElement && !e.shiftKey) {
      ...
      if (submitButton) {
        ...
        const suggestedText = suggestionPopup?.querySelector?.('.twitter-improver-suggested-text')?.textContent;
  ...
  const observer = new MutationObserver((mutations) => {
      ...
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === 1) { // Element node
          ...
          if (node.querySelector) {
            const inputs = node.querySelectorAll('[data-testid="tweetTextarea_0"], [data-testid="tweetTextarea"], [contenteditable="true"], [aria-label="Tweet text"], [aria-label="Post text"]');
    ...
    if (newComposeAreaFound) {
      setTimeout(() => {
        ...
        const activeInputs = document.querySelectorAll('[data-testid="tweetTextarea_0"] [contenteditable="true"], [data-testid="tweetTextarea"] [contenteditable="true"]');
